...
export type GraphVisualizerOptions = {
  datasetName?: string;
  backHref?: string;
};

export class GraphVisualizer {
  generateToHtml(result: AnalysisResult, options?: GraphVisualizerOptions): string {
    const { nodes, edges } = this.buildNodesAndEdges(result);
    const recommended = computeRecommendedFilters(edges);
    return this.generateHtml(nodes, edges, result.stats, recommended, options);
  }
...
<body>
  <div id="header">
    ${backLink}
    <h1>🎯 Kairo - Salesforce Metadata Dependency Graph${title}</h1>
    <div id="stats">
      <div class="stat">
